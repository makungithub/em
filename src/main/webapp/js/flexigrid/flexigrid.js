...
(function($) {
	...
	$.addFlex = function(t, p) {
		...
		p = $.extend({
			...
			checkbox : false,// 是否要多选框<---qc修改 如果为false则显示单选框 暂时这么放 以后有需要再在上边加第三种属性--->
					...
					$.each(data.rows, function(i, row) {
						    ...
						    if (p.colModel) {
							    for (j = 0; j < p.colModel.length; j++) {
								    ...
								    var func = "";
								    var paramArray = null;
								    if(cm.action != null && cm.action != 'undefined'){
								    	var action = cm.action;
								    	//把动作分成函数名和参数两部分并去掉空格
								    	var param = action.substring(action.indexOf('(')+1,action.indexOf(')')).replace(/(^\s*)|(\s*$)/g,"");
								    	func = action.substring(0,action.indexOf('('));
								    	if(param!=""){
								    ...
								    $.each(data.rows[i], function(x, y) {
									        ...
									        if(func!=""){
									        	...
									        	if(paramArray!=null&&paramArray.length!=0){
									        		for(var o=0;o<paramArray.length;o++){
									        			var paramter = paramArray[o].replace(/(^\s*)|(\s*$)/g,"");
									        			//如果参数不是常量则进行遍历去除对应的变量值
									        			if(paramter.indexOf("'")==-1||paramter.indexOf("\"")==-1){
									        				if(paramter == x){
									        					paramArray[o] = "'"+y+"'";
									        				}
									        			}
								    ...
								    //add pophints
								    if(cm.pophint != null && cm.pophint != 'undefined'){
								    	pophints.push(cm.pophint);
								    }else {
								    	pophints.push("");
								    }
							    }
						    ...
						    $('thead tr:first th', g.hDiv).each(function() {
							        ...
							        //如果有动作存在
							        if(tdAction[idx] != null && tdAction[idx] != ""){
							        	//在列中新增一个链接并把需要执行的方法进行绑定
							        	var a = $("<a></a>");
							        	a.html(tdVal[idx]);
							        	a.attr("href","#");
							        	a.click(function (){
							        	...
							        	a.attr("onclick",tdAction[idx]);
							        	$(td).append(a);
							        }else{
							        	//add pophints
//							        	alert(tdVal[idx] + " :: " +pophints[idx]+ " :: " +idx);
							        	 if(pophints[idx] != null && pophints[idx] != ""){
								        	if(pophints[idx] == true){
								        		td.innerHTML = "<p style = \" margin-top: 0px; margin-bottom: 0px;\" title=\"" +tdVal[idx]+"\">" +tdVal[idx] +"</p>";
								        	} else {
	...
	//							    cthch.addClass("noborder").click(function() {//add matychen 不要noborder，加入后ie8显示有问题。
							    	...
							    	var cth = $('<th/>');
								    var cthch = $('<input type="radio" value="" name="radioGroup"' 
								        + $(tr).attr('id') + '"/>');
								    var objTr = $(tr);
								    cthch.click(function() {
									        if (this.checked) {
			...
			populate : function() { // get latest data
				...
				//把数据进行拼接,目前仅支持json型数据扩展  
				if(p.dataType=='json'){
					if(p.queryDate!=''){
						if(p.queryDate){
							for(var x in p.queryDate){
								//可以考虑在这里做判断,如果传进来的参数为空则不放到参数传递到后台
								//目的是后台拿到paramMap可以直接扔到公用manager去查询而不用做任何此操作(未实现,只是有这个想法)
								param[param.length] = {
									    name : x,
									    value : p.queryDate[x]
								    };
		...
		if (p.colModel) {
			...
			for (i = 0; i < p.colModel.length; i++) {
				if(typeof(p.colModel[i]) != "undefined"){
					var cm = p.colModel[i];
		...
		if (p.buttons) {
			...
			for (i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					//Add by WLB 2010-11-05 AM 
					//增加flexgrid按钮id属性 ,并且在绑定事件时用id属性判断
					btnDiv.id = btn.id;
			...
			if(p.checkbox != "none"){
				...
				if (p.checkbox) {
					...
					$('tr', g.hDiv).each(function() {
						...
						var cthch = $('<span style="padding-left:17px;">&nbsp;</span>');
	...
	//绑定查询form
	$.fn.queryWithFrom = function (formObj){
		if(formObj == null)
			return null;
		return this.each(function() {
			this.p.queryDate = formObj.serializeObject();
			if (this.grid && this.p.url)
			    this.grid.populate();
		});
	}
	...
	//通过指定json对象进行查询
	//数据格式为{"search_productname":"111","search_productcode":"222"}
	$.fn.queryWithJSON = function (data){
		if(data == null)
			return null;
		return this.each(function() {
			this.p.queryDate = data;
			if (this.grid && this.p.url)
				this.grid.populate();
		});
	}
